...
import { ROLE, RoleName } from '../shared/src/model/roles';
...
const saveUserRoles = async (user: User) => {
  const customClaims = Object.entries(user.roles).reduce((acc: CustomClaims, [wiki, role]: [string, ROLE]) => {
...
export const getCommandModules = (container: Container): ObjectOf<CommandModule> => {
  return {
    setrole: {
      ...
      handler: async (args: Arguments) => {
        ...
        const newRole: ROLE | undefined = args.role === NONE_ROLE ? undefined : ROLE[args.role as RoleName];
