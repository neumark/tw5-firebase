...
import { User, username } from '../../../shared/src/model/user';
import { Config } from '../../../shared/src/model/config';
import { CallbackFn, SyncAdaptor, SyncAdaptorTiddlerInfo, TW5Tiddler, Wiki } from '../../tw5-shared/src/tw5-types';
  ...
  saveTiddler(tiddler: TW5Tiddler, callback: (err: any, ...data: any[]) => void) {
    asyncToCallback(async () => {
      ...
      const writeResult = await (originalBag
        ? this.store.update(originalBag, title, tiddlerData, expectedRevision)
  ...
  deleteTiddler(
    ...
    asyncToCallback(async () => {
      ...
      if (bag) {
        ...
        await this.store.del(bag, title, this.tiddlerRevision[title]);
      }
