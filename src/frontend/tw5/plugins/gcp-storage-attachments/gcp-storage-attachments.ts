...
import { TW5ImportFileInfo } from "../../tw5-types";
  ...
  const getWikiName = () => maybeApply((str:string) => JSON.parse(str), $tw.wiki.getTiddlerText('$:/config/WikiConfig'))?.wiki?.wikiName;
  ...
  const shouldUploadToStorage = (info:TW5ImportFileInfo) => $tw.wiki.getTiddlerText(ENABLE_EXTERNAL_ATTACHMENTS_TITLE,"") === "yes" && (info.isBinary || info.file.size > MAX_SIZE);

  const getURL = (metadata:any) => `https://firebasestorage.googleapis.com/v0/b/${metadata.bucket}/o/${encodeURIComponent(metadata.fullPath)}?alt=media`;
