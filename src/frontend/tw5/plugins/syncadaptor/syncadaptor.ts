...
class TW5FirebaseSyncAdaptor implements SyncAdaptor {
  ...
  constructor(options: { wiki: Wiki }) {
    ...
    const namespacedTiddlers: SingleWikiNamespacedTiddler[] = ($tw as any)
      ._pnwiki.namespacedTiddlers;
    this.tiddlerInfo = namespacedTiddlers.reduce(
      (acc, { tiddler: { title }, bag }) => {
        acc[title] = { bag };
        return acc;
      },
      {} as { [title: string]: SyncAdaptorTiddlerInfo }
    );
    this.tiddlerRevision = namespacedTiddlers.reduce(
      (acc, { tiddler: { title }, revision }) => {
        acc[title] = revision;
        return acc;
      },
...
export const adaptorClass = $tw.wiki.tiddlerExists(CONFIG_TIDDLER)
  ? TW5FirebaseSyncAdaptor
  : null;
