...
export class TransformingTiddlerPersistence<
  OuterTiddlerFormat,
  InnerTiddlerFormat
> implements
    Persistence<string, Revision, OuterTiddlerFormat, TiddlerNamespace> {
  ...
  private backingPersistence: Persistence<
    string,
    Revision,
    InnerTiddlerFormat,
    TiddlerNamespace
  ...
  private innerToOuter: (inner: {
    namespace: TiddlerNamespace;
    key: string;
    value: InnerTiddlerFormat;
    revision: Revision;
  }) => {
    namespace: TiddlerNamespace;
    key: string;
    value: OuterTiddlerFormat;
    revision: Revision;
  };
  ...
  constructor(
    ...
    backingPersistence: Persistence<
      string,
      Revision,
      InnerTiddlerFormat,
      TiddlerNamespace
    ...
    this.innerToOuter = (inner: {
      namespace: TiddlerNamespace;
      key: string;
      value: InnerTiddlerFormat;
      revision: Revision;
    }) => ({ ...inner, value: this.decode(inner.value) });
  }
  ...
  > {
    return (await this.backingPersistence.readDocs(documentKeys)).map(
      this.innerToOuter
    );
  }
  ...
  > {
    return (await this.backingPersistence.readCollections(namespaces)).map(
      this.innerToOuter
    );
  }
  async updateDoc(
    ...
    updater: (
      value?: OuterTiddlerFormat
    ) => MaybePromise<OuterTiddlerFormat | undefined>,
  ...
  > {
    ...
    return innerResult
      ? {
          namespace,
          key,
          revision: innerResult.revision,
          value: this.decode(innerResult.value),
        }
      : undefined;
  }
}
