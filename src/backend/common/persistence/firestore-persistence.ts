...
const asyncMap = async <T, R = any>(list: T[], fn: (e: T) => Promise<R>) => await Promise.all(list.map(fn));
...
export class FirestorePersistence implements TiddlerPersistence {
    ...
    toFirestoreTiddler(tiddler: Tiddler, revision: Revision): FirestoreSerializedTiddler {
    ...
    async createTiddler(namespace: TiddlerNamespace, tiddler: Tiddler, revision: Revision): Promise<void> {
        ...
        this.tx.set(this.db.doc(makeKey(namespace, tiddler.title)), this.toFirestoreTiddler(tiddler, revision));
        return;
    }
}
